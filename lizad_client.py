...
class LiZADClient:
    ...
    def run_multi_scores(self, frame_rgb, timeout=5.0):
        """Scores frame_rgb against every configured prompt set on the server.
        Returns {label: score} - lighter than run(), no anomaly maps transferred,
        meant for batch comparison of prompt sets rather than live display."""
        frame_bgr = cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2BGR)
        _, jpg_bytes = cv2.imencode(".jpg", frame_bgr)
        image_b64 = base64.b64encode(jpg_bytes.tobytes()).decode("utf-8")

        resp = requests.post(f"{self.base_url}/infer_multi", json={"image_b64": image_b64}, timeout=timeout)
        resp.raise_for_status()
